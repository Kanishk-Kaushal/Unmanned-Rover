...
#define move_link1(x) TIM3->CCR3 = (x); // B0
#define move_link2(x) TIM3 ->CCR4 = (x); // B1

#define stop_roll TIM2->CCR1 = 0;
#define move_roll TIM2->CCR1 = pwm; // A0

#define stop_pitch TIM2->CCR2 = 0;
#define move_pitch TIM2->CCR2 = pwm; // A1

#define stop_gripper TIM3->CCR1 = 0;
#define move_gripper TIM3->CCR1 = pwm; // A6

#define stop_allen GPIOA -> BSRR = 1 << (7+16); // A7
#define move_allen GPIOA -> BSRR = 1 << 7;

#define stop_mast TIM4 -> CCR4 = 0; //B9
#define move_mast TIM4 -> CCR4 = pwm;
...
void GPIO_Initialize()
{
	...
	// UPDATE -------------------------------------------------- //
	// PB15 DIR
	GPIOB->CRH |= GPIO_CRH_MODE15;	 //OUTPUT Mode (11)
	GPIOB->CRH &= ~(GPIO_CRH_CNF15); //Output Push-Pull (00)

	// b14  MAST CAM DIR
	GPIOB->CRH |= GPIO_CRH_MODE14;	 //OUTPUT Mode (11)
	GPIOB->CRH &= ~(GPIO_CRH_CNF14); //Output Push-Pull (00)

	// b13 LIMIT SWITCH
	GPIOB->CRH |= GPIO_CRH_MODE13;	 //OUTPUT Mode (11)
	GPIOB->CRH &= ~(GPIO_CRH_CNF13); //Output Push-Pull (00)
	...
	GPIOA->CRL |= GPIO_CRL_CNF6_1; //AF Output Push-Pull (10)
...
void Timer_Initialize() {
	...
	TIM4->CCER |= TIM_CCER_CC1E | TIM_CCER_CC2E | TIM_CCER_CC4E; //Enable Channel 1 and 2 as OUTPUT
...
{
	if (DL == 1)
		GPIOA->BSRR |= 1 << 4;   //Turn on LEFT LED  A4 LEFT WHEEL DIR
	else
		GPIOA->BRR |= 1 << 4;   //Turn off LEFT LED  A5 DIR RIGHT WHEEL
	...
	TIM4->CCR1 = (uint32_t) abs(abs(a * X) - abs(b * Y)) * (gear * 0.1); //Left PWM  // B6 LEFT WHEEL
	TIM4->CCR2 = (uint32_t) abs(abs(p * X) - abs(q * Y)) * (gear * 0.1); //Right PWM // B7 RIGHT WHEEL
}
